...
The UDP protocol reaveales that the DNS server is unreachable over port 53. This is based on the results of the network analysis, which shoe that the ICMP echo reply retuned the error message "udp port 53 unreachable." The port noted in the error message is used TCP and UDP communication. It is likely that the DNS server is not responding

### Part 2: Explain your analysis of the data and rpovide at least one cause of the incident

Time incident occured this afternoon at 1:24 PM when several customers of clients reported that they were not able to access the website. Once the department was made aware of this issue, we visited the website oursevles to verify and it was indeed down. We then loaded up tcpdump to begin testing the website and recording its response. The logs revealed that port 53, whcih is used for TCP and UDP communicatoin, was not reachable. We will conduct an investigation to detemine the root cause for this issue. In the meantime, we will be checking the firewall configuration to see if port 53 is blocked and contacting system administrarots for the web server to have them check the system for any signs of attacks. 
